...
-- ----------------------------
--  Table structure for `web_environment`
-- ----------------------------
-- DROP TABLE IF EXISTS `web_environment`;
CREATE TABLE `web_environment` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'id 自增',
  `systemId` int(11) DEFAULT NULL COMMENT '所属系统',
  ...
  `totalJSHeapSize` int(11) NOT NULL DEFAULT '0' COMMENT 'JS堆栈总大小',
  `jsHeapSizeLimit` int(11) NOT NULL DEFAULT '0' COMMENT 'JS的堆栈限制',
  `usedJSHeapSize` int(11) NOT NULL DEFAULT '0' COMMENT 'JS堆栈使用大小',
  `usedLocalStorageSize` int(11) NOT NULL DEFAULT '0' COMMENT 'Localstorage使用大小',
  `usedLocalStorageKeys` int(11) NOT NULL DEFAULT '0' COMMENT 'Localstorage的key数量',
  `usedSessionStorageSize` int(11) NOT NULL DEFAULT '0' COMMENT 'SessionStorage使用大小',
...
CREATE TABLE `web_pages` (
  ...
  `domTime` int(11) NOT NULL COMMENT 'DOM构建时间 单位：ms',
  `resourceTime` int(11) NOT NULL DEFAULT '0' COMMENT '资源加载耗时 单位：ms',
  ...
  `connectEnd` int(20) NOT NULL DEFAULT '0' COMMENT '完成建立连接时的时间戳',
  `connectStart` int(20) NOT NULL DEFAULT '0' COMMENT '开始建立连接时的时间戳',
  `bodySize` int(11) NOT NULL DEFAULT '0' COMMENT '解码后Body大小',
  `domainLookupEnd` int(20) NOT NULL DEFAULT '0' COMMENT '域名查询结束的时间戳',
  `domainLookupStart` int(20) NOT NULL DEFAULT '0' COMMENT '域名查询开始的时间戳',
  `domComplete` int(20) NOT NULL DEFAULT '0' COMMENT '文档解析完成的时间戳',
  `domContentLoadedEventEnd` int(20) NOT NULL DEFAULT '0'  COMMENT '所有需要立即执行的脚本已经被执行（不论执行顺序）时的时间戳',
  `domContentLoadedEventStart` int(20) NOT NULL DEFAULT '0' COMMENT '所有需要被执行的脚本已经被解析时的时间戳',
  `domInteractive` int(20) NOT NULL DEFAULT '0' COMMENT 'DOM结构结束解析、开始加载内嵌资源时的时间戳',
  `domLoading` int(20) NOT NULL DEFAULT '0' COMMENT 'DOM结构开始解析时的时间戳',
  `encodedBodySize` int(11) NOT NULL DEFAULT '0' COMMENT '编码后的body大小',
  `fetchStart` int(20) NOT NULL DEFAULT '0' COMMENT '浏览器准备好使用HTTP请求来获取(fetch)文档的时间戳',
  `loadEventEnd` int(20) NOT NULL DEFAULT '0' COMMENT '加载事件完成时的时间戳',
  `loadEventStart` int(20) NOT NULL DEFAULT '0' COMMENT 'load事件被发送时的时间戳',
  `navigationStart` int(20) NOT NULL DEFAULT '0' COMMENT '上一个文档卸载(unload)结束时的时间戳',
  ...
  `requestStart` int(20) NOT NULL DEFAULT '0' COMMENT '发出HTTP请求时（或开始读取本地缓存时）的时间戳',
  `responseEnd` int(20) NOT NULL DEFAULT '0' COMMENT '从服务器收到最后一个字节时的时间戳',
  `responseStart` int(20) NOT NULL DEFAULT '0' COMMENT '从服务器收到第一个字节时的时间戳',
  `transferSize` int(11) NOT NULL DEFAULT '0' COMMENT '数据包大小',
  `unloadEventEnd` int(20) NOT NULL DEFAULT '0' COMMENT 'unload事件处理完成时的时间戳',
  `unloadEventStart` int(20) NOT NULL DEFAULT '0' COMMENT 'unload事件抛出时的时间戳',
  ...
  `main_restiming` LongText DEFAULT NULL COMMENT '主应用瀑布流数据',
  ...
  `sessionStart` int(20) NOT NULL DEFAULT '0' COMMENT 'Session开始时间戳',
  `triggerMethod` varchar(20) DEFAULT NULL COMMENT '触发时间戳',
  `triggerStart` int(20) NOT NULL DEFAULT '0' COMMENT '触发时间戳',
